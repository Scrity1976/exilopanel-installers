...
# Official Sentora/ExiloPanel Automated Installation Script
# =============================================
#
...
echo ""
echo "############################################################"
echo "#  Welcome to the Official ExiloPanel Installer $SENTORA_INSTALLER_VERSION  #"
echo "############################################################"
...
if [[ "$OS" = "CentOs" && ("$VER" = "6" || "$VER" = "7" ) || 
    ...
    echo "Sorry, this OS is not supported by ExiloPanel." 
    exit 1
fi
...
# Check for some common control panels that we know will affect the installation/operating of ExiloPanel.
if [ -e /usr/local/cpanel ] || [ -e /usr/local/directadmin ] || [ -e /usr/local/solusvm/www ] || [ -e /usr/local/home/admispconfig ] || [ -e /usr/local/lxlabs/kloxo ] ; then
    ...
    echo "is designed to install and configure ExiloPanel on a clean OS installation only."
    echo -e "\nPlease re-install your OS before attempting to install using this script."
    exit 1
fi

# Check for some common packages that we know will affect the installation/operating of ExiloPanel.
if [[ "$OS" = "CentOs" ]] ; then
...
if [ -L "/etc/zpanel" ] && [ -d "/etc/zpanel"  ]; then
    ...
    if [ $pkginst = "y" ]; then
        ...
        echo "$pkginstlist"
        echo "This installer is designed to install and configure ExiloPanel on a clean OS installation only!"
        echo -e "\nPlease re-install your OS before attempting to install using this script."
        exit 1
    fi
    unset pkginst
...
#   -t <timezone-string>
# like :
#   exilopanel_install.sh -t Europe/Paris -d panel.domain.tld -i xxx.xxx.xxx.xxx
# notes:
...
if [[ "$PANEL_FQDN" == "" ]] ; then
    ...
    echo ' 1) the sub-domain that you want to use to access ExiloPanel,'
    ...
    while true; do
        echo ""
        read -e -p "Enter the sub-domain you want to access ExiloPanel: " -i "$PANEL_FQDN" PANEL_FQDN
        ...
        dns_panel_ip=$(host "$PANEL_FQDN"|head -n1|cut -d" " -f4)
        if [[ "$dns_panel_ip" == "" ]]; then
            ...
            get_main_domain=$(echo $PANEL_FQDN | cut -d "." -f2-)
            if [[ "$dns_panel_ip" != "$PUBLIC_IP" && "$(host -t ns "$get_main_domain")" != *"ns.cloudflare.com"* ]]; then
                echo -e -n "\e[1;31mWARNING: $PANEL_FQDN DNS record does not point to $PUBLIC_IP!\e[0m"
                echo "  ExiloPanel will not be reachable from http://$PANEL_FQDN"
                confirm="true"
            fi
        fi
        ...
        if [[ "$PUBLIC_IP" != "$extern_ip" && "$PUBLIC_IP" != "$local_ip" ]]; then
            ...
            echo "  ExiloPanel will not work with this IP..."
                confirm="true"
        fi
        ...
        if [[ "$confirm" != "" ]] ; then
            ...
            echo "Are you really sure that you want to setup ExiloPanel with these parameters?"
            ...
            read -e -p "All is ok. Do you want to install ExiloPanel now (y/n)? " yn
            case $yn in
                [Yy]* ) break;;
                [Nn]* ) exit;;
            esac
...
echo "ExiloPanel core version $SENTORA_CORE_VERSION"
echo ""
echo "Installing ExiloPanel $SENTORA_CORE_VERSION at http://$PANEL_FQDN and ip $PUBLIC_IP"
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    # Removal of conflicting packages prior to ExiloPanel installation.
...
#--- Install utility packages required by the installer and/or ExiloPanel.
...
#--- Download ExiloPanel archive from GitHub
echo -e "\n-- Downloading ExiloPanel, Please wait, this may take several minutes, the installer will continue after this is complete!"
# Get latest ExiloPanel
while true; do
    wget -nv -O sentora_core.zip https://github.com/Scrity1976/exilopanel-core/releases/download/v$SENTORA_CORE_VERSION/exilopanel-core-$SENTORA_CORE_VERSION.zip
    if [[ -f sentora_core.zip ]]; then
        break;
    else
        echo "Failed to download ExiloPanel core from Github"
...
if [ ! -L "/etc/zpanel" ] && [ -d "/etc/zpanel" ]; then
    ...
    rm -rf "$PANEL_PATH/exilopanel-core-$SENTORA_CORE_VERSION/cnf"
 
fi

## cp can be aliased to stop overwriting of files in centos use full path to cp
/bin/cp -rf "$PANEL_PATH/exilopanel-core-$SENTORA_CORE_VERSION/." "$PANEL_PATH/panel/"
rm sentora_core.zip
...
#--- Set-up ExiloPanel directories and configure permissions
...
#--- Install preconfig
while true; do
    wget -nv -O sentora_preconfig.zip https://github.com/Scrity1976/sentora-installers/archive/$SENTORA_INSTALLER_VERSION.zip
    if [[ -f sentora_preconfig.zip ]]; then
        break;
    else
        echo "Failed to download ExiloPanel preconfig from Github"
        echo "If you quit now, you can run again the installer later."
        read -e -p "Press r to retry or q to quit the installer? " resp
        case $resp in
            [Rr]* ) continue;;
...
#-----------------------------------------------------------
# Install all softwares and dependencies required by ExiloPanel.
...
sudo add-apt-repository 'deb [arch=amd64,i386,ppc64el] http://mariadb.mirror.serveriai.lt/repo/10.3/debian jessie main'
else
if [ $UPGRADE_MYSQL == true ]; then
...
# setup ExiloPanel access and core database
...
# remove default virtual site to ensure ExiloPanel is the default vhost
if [[ "$OS" = "CentOs" ]]; then
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    $PACKAGE_INSTALLER libapache2-mod-php7.2 php7.2-common php7.2-cli php7.2-mysql php7.2-gd php7.2-mbstring php7.2-curl php-pear php7.2-imap php7.2-xmlrpc php7.2-xsl php7.2-intl
    if [ "$VER" = "14.04" ]; then
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    sed -i "s|;session.save_path = \"/var/lib/php/7.2\"|session.save_path = \"$PANEL_DATA/sessions\"|" $PHP_INI_PATH
fi
...
# Setup proftpd base file to call ExiloPanel config
rm -f "$FTP_CONF_PATH"
#touch "$FTP_CONF_PATH"
...
#--- Set some ExiloPanel database entries using. setso and setzadmin (require PHP)
echo -e "\n-- Configuring ExiloPanel"
...
#--- Advise the admin that ExiloPanel is now installed and accessible.
{
echo "########################################################"
echo " Congratulations ExiloPanel has now been installed on your"
echo " server. Please review the log file left in /root/ for "
echo " any errors encountered during installation."
echo ""
echo " Login to ExiloPanel at http://$PANEL_FQDN"
echo " ExiloPanel Username  : zadmin"
echo " ExiloPanel Password  : $zadminpassword"
echo ""
echo " MySQL Root Password      : $mysqlpassword"
echo " MySQL Postfix Password   : $postfixpassword"
echo " MySQL ProFTPd Password   : $proftpdpassword"
